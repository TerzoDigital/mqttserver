...
```
Using *CTRL-C* to end the follow of the logs. Or do not use the -f if you do not want to follow. If the logs are long then the entire set of logs will be listed before you get to the end. If you would prefer to just say the last 10 lines of logs then follow, use the command:

```
docker compose logs -f --tail 10 mosquitto
```
...
If you have changed config or need to restart the broker then the service can be quickly restarted with:
...
During the execution of the *initial-setup.sh* script, the script *generate-certs.sh* is also run. This sets up the broker to use locally generated certificates and provides all the relevant files to do that. Specifically it does the following:
...
Note that a single client key and certificate are generated, ready for use by one or more clients in mutual TLS (mTLS). More client certificates and keys can be generated by using the instructions within the script file to generate more certificates, or use the provided script which is [described below](#generating-keys-and-certificates-for-more-users).

The contents of the mosquitto.conf file related to TLS configuration should read as follows:
...
# Require clients to present valid certs signed by our CA
require_certificate true
use_identity_as_username true # set true if you want CN from cert to act as username
```

> [!IMPORTANT]
>When mTLS is in use, and assuming the MQTT client is configured to check the servers certificate (which can sometimes be turned off) then the certificate for the MQTT server must have a CN which is the hostname of the server. If it does not then it will fail validation at the client. The *generate-certs.sh* script provided with this *readme.md* would need changed to have the correct hostname in the CN field when generating the certificate request. Alternatively the commands can be manually run to create another server certificate and key, then the *mosquitto.conf* file can be changed to point to those files.
